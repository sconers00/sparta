...
public class MemberService {
    ...
    @Transactional
    public LogInResponseDto login(@NotBlank String email, @NotNull String password){//로그인 처리기
        Optional<Member> optionalMember = memberRepository.findMemberByEmailAndPassword(email, password);//이메일과 비밀번호가 일치하는 회원을 검색
        if(optionalMember.isEmpty()){throw new MismatchException(HttpStatus.UNAUTHORIZED,"There is no matching combination of email and password.");}//일치하는 회원이 없다면 401
        Member findMember = optionalMember.get();
        return new LogInResponseDto(findMember.getEmail(), findMember.getUserid());//userid 와 email 반환
    }
